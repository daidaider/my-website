...
function renderCard(item, page) {
  ...
  const rows = isOrchard ? [detail && `<p class="place-info">${escapeHtml(detail)}</p>`] : [item.address && `<p class="place-info">${escapeHtml(item.address)}</p>`, item.hours && `<p class="place-info"><strong>營業時間：</strong>${escapeHtml(item.hours)}</p>`, item.closed && `<p class="place-info"><strong>公休日：</strong>${escapeHtml(item.closed)}</p>`, detail && `<p class="place-info">${escapeHtml(detail)}</p>`];
  const actions = [`<a class="place-action" href="${map}" target="_blank" rel="noopener">Google 地圖</a>`];
  if (item.phone) actions.push(`<a class="place-action" href="${phoneUrl(item.phone)}">撥打 ${escapeHtml(item.phone)}</a>`);
  ...
  return `<article class="place-card"><p class="place-type">${isOrchard ? '紅棗園' : escapeHtml(item.type)}</p><h3><a href="${map}" target="_blank" rel="noopener">${title}</a></h3>${rows.filter(Boolean).join('')}<div class="place-actions">${actions.join('')}</div></article>`;
}
